    ...
    return [
        {
            ...
            "log_level": "ERROR",
            "error_message": "Database connection timeout while processing card charge",
            "stack_trace": "DBConnectionTimeoutException at PaymentRepository.save",
            "resolution_notes": "Restart PostgreSQL and increase the timeout",
            "severity": "HIGH",
        },
        {
            "incident_id": "INC101",
            ...
            "resolution_notes": "Restart Redis and flush the client pool",
            "severity": "HIGH",
        },
    ]
...
@pytest.fixture()
def settings(tmp_path: Path, sample_incidents: list[dict[str, str]]) -> Settings:
    data_dir = tmp_path / "data"
    sample_path = tmp_path / "seed_incidents.json"
    sample_path.write_text(json.dumps(sample_incidents), encoding="utf-8")
    settings = Settings(
        debug=True,
        data_dir=data_dir,
